<section class="relative min-h-screen py-16 bg-gray-200 min-w-screen animation-fade animation-delay">
    <div class="container w-full px-10 mx-auto sm:px-5">
        <div class="sm:flex">
                ...
                @foreach ($team->availablePlans() as $plan)
                    ...
                    <div @click="toggle = {{ $plan->id }}"
                        ...
                        <div class="flex-grow px-0 py-5 md:px-5">
                            <div class="text-md lowercase">{{ $plan->title }}</div>
                        </div>
            ...
            @foreach ($team->availablePlans() as $plan)
                ...
                <div x-show="toggle === {{ $plan->id }}" class="flex w-full mt-10 bg-white rounded-md sm:mt-1 sm:ml-5 sm:flex-initial">
                    <div class="flex-grow px-4 py-5 w-5/12">
                        <div class="text-xs font-semibold">{{ $plan->title }}</div>
                        <div class="flex mt-5">
                    ...
                    @if($plan->banner)
                        <div class="flex w-7/12 h-full bg-blue-500 bg-opacity-25 rounded-r md:flex-shrink bg-cover" style="background: url('{{ $plan->banner }}');"></div>
                        @else
                        <div class="flex w-7/12 h-full bg-blue-500 bg-opacity-25 rounded-r md:flex-shrink bg-cover" style="background: url('{{ $team->logo }}');"></div>
                    @endif
                </div>
