<template>
        <div class="w-full xl:w-4/12 px-3">
            ...
            <div class="flex items-center bg-white rounded-lg text-gray-800 shadow transition duration-500 ease-in-out hover:shadow-lg">
                <!--Initials-->
                <InertiaLink :href="link || '#'" class="cursor-pointer h-full w-2/12 py-5 px-8 rounded-l-lg text-white flex items-center justify-center" :class="randomColor">{{ initials }}</InertiaLink>
                <!--End Initials-->
                <!--Text -->
                <InertiaLink :href="link || '#'" class="cursor-pointer px-3 mr-auto py-0 my-0">
                    <h4 class="text-gray-800 font-semibold text-xs lg:text-sm">{{ excerpt(title) }}</h4>
                    <small class="text-xs text-gray-500 text-xs lg:text-sm">{{ excerpt(info) }}</small>
                </InertiaLink>
                <!--End Text-->
                    ...
                    <transition name="fade">
                        <template v-if="isOpen">
                            ...
                            <div class="options absolute bg-white text-gray-600 origin-top-right right-0 mt-6 w-56 rounded-md shadow-lg overflow-hidden z-10">
                                <!--Edit Icon-->
                                <InertiaLink href="#" class="flex py-3 px-2 text-sm font-bold hover:bg-gray-100">
                                ...
                                <InertiaLink href="#" class="flex py-3 px-2 text-sm font-bold bg-red-400 text-white">
