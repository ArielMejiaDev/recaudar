...
class UpdateTeamLegalDataTest extends TestCase
{
    ...
    /** @test */
    public function test_ap_admins_cannot_update_team_legal_data_without_valid_data()
    {
        $user = factory(User::class)->create();
        $team = factory(Team::class)->create(['name' => 'recaudar']);
        ...
        $anotherTeam = factory(Team::class)->create([
            ...
            'tax_number' => 12345678,
            'use_of_funds' => 'Lorem ipsum dolor siet ...',
        ]);
        $response = $this->put(route('admin.teams.update-legal-data', $anotherTeam), [
        ...
        $response->assertSessionHasErrors(['legal_representative', 'tax_number', 'use_of_funds']);
        $anotherTeam->refresh();
        $this->assertNotEquals('', $anotherTeam->legal_representative);
    }
    /** @test */
    public function test_ap_admins_can_update_team_legal_data()
    {
        $user = factory(User::class)->create();
        $team = factory(Team::class)->create(['name' => 'recaudar']);
        $team->users()->attach($user, ['role_name' => 'app_admin']);
        ...
        $response = $this->put(route('admin.teams.update-legal-data', $anotherTeam), [
            ...
            'use_of_funds' => 'Lorem ipsum dolor siet ...updated',
        ]);
